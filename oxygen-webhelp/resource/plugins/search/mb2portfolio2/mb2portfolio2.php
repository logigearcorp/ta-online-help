 ...
 * @since  1.6
 */
class PlgSearchMb2portfolio2 extends JPlugin
{
	/**
	...
	public function onContentSearch($text, $phrase = '', $ordering = '', $areas = null)
	{
		...
		require_once JPATH_SITE . '/components/com_mb2portfolio2/helpers/route.php';
		...
		if ($sContent && $limit > 0)
		{
			...
			$query->select('a.title AS title, a.metadesc, a.metakey, a.created AS created, a.language, a.catid')
				...
				->from('#__mb2portfolio2_items AS a')
			...
			if (isset($list))
			{
				...
				{
					$list[$key]->href = Mb2portfolio2HelperRoute::getArticleRoute($item->slug, $item->catid, $item->language);
				}
			}
		...
		if ($sArchived && $limit > 0)
		{
			...
			$query->from('#__mb2portfolio2_items AS a')
			...
			$item = $app->getMenu()->getItems('link', 'index.php?option=com_mb2portfolio2&view=archive', true);
			...
			if (isset($list3))
			{
				foreach ($list3 as $key => $item)
				{
					...
					$list3[$key]->href = JRoute::_('index.php?option=com_mb2portfolio2&view=archive&year=' . $created_year . '&month=' . $created_month . $itemid);
				}
			}
